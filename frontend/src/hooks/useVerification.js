// src/hooks/useVerification.js - UPDATED FOR SINGLE SERVER
...
export function useVerification() {
  ...
  // Start verification - PIN OR OTP
  const startVerification = useCallback(async (userData, type = "otp") => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (type === "pin") {
        ...
        result = await verificationService.requestPinVerification(userData);
      } else {
        // For OTP verification (default)
        result = await verificationService.requestVerification(userData);
      }
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        // Check if it's a PIN session or OTP session
        if (sessionId.startsWith("PIN_")) {
          data = await verificationService.checkPinStatus(sessionId);
        } else {
          data = await verificationService.checkStatus(sessionId);
        }
